...
class MarkovModel:
    ...
    def sentenceLogProbability(self, s, alpha):
        ...
        for i in range(1, len(sent)):
            a = self.prob(sent[i], tuple(sent[max(0,i-self.K+1):i]), alpha)
            if a <= 0:
                print("Sentence i: " + sent[i])
                print("Full sentence: " + sent.__str__())
                print("Original sentence: " + s.__str__())
                print("Context: " + tuple(sent[max(0,i-self.K+1):i]).__str__())
            else: 
                b = math.log(a, 2)
                summ += b
