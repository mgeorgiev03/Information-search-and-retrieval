...
if len(sys.argv)>1:
    if sys.argv[1] == '3':
        try:
            ...
            assert len(seq) == 1233319, "Броят на елементите на seq не съвпада с очаквания, a sa: '{}'".format(len(seq))
        ...
        try:
            ...
            assert np.abs(np.sum(np.exp(q_noise)/10) - 1.0) < 1e-7, "Сумата от вероатностите за контекстните думи следва да е 1.0, a e '{}'".format(np.abs(np.sum(np.exp(q_noise)/10) - 1.0))
