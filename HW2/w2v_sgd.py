...
def stochasticGradientDescend(data, U0, V0, W0, contextFunction, lossAndGradientFunction, q_noise, batchSize=1000, epochs=1, alpha=1., printInterval=10000):
    ...
    while epoch<epochs:
        ...
        for b in range(0,len(idx),batchSize):
            ...
            q = np.stack( [[q_noise[c] for c in context]  for _,context in batchData] ) 
            ...
            idx_u = np.array([w for w, _ in batchData])
            idx_v = np.array([c for _, c in batchData])

            W -= alpha * np.array(dW)

            U[idx_u] = -alpha * np.array(du_w)
            V[idx_v] = -alpha * np.array(dVt)
